...
import { mockAssistantAnsweringMessage, mockAssistantDoneMessage, mockAssistantThinkingMessage } from "@/data/mock-chat";
...
export function EmptyState() {
  ...
  const previewMessage: Message | null = previewStage === "connecting"
  ...
  return (
    <section className="mx-auto flex w-full max-w-3xl flex-1 flex-col items-center justify-center px-4 pb-44 pt-14 text-center sm:px-8">
      ...
      {import.meta.env.DEV && <div className="mt-7 w-full text-left">
        <div className="mb-4 flex flex-wrap items-center justify-center gap-1.5" aria-label="Preview stages">
          <span className="mr-1 text-xs font-medium text-muted-foreground">Preview stages</span>
          {previewStages.map((stage) => <Button key={stage} variant={previewStage === stage ? "soft" : "ghost"} size="sm" className="h-8 capitalize" onClick={() => setPreviewStage(stage)}>{stage}</Button>)}
        </div>
        {previewMessage && <div className="rounded-lg border border-border/70 bg-background/80 p-4 sm:p-5"><ChatMessage key={previewStage} message={previewMessage} /></div>}
      </div>}
    </section>
  );
}